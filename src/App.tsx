...
const App = () => {
  ...
  const handleReview = async (resumeText: string, jobTitle: string) => {
  ...
  return (
    <main className="relative min-h-screen w-full bg-slate-900 text-slate-100">
      {/* Header */}
      <header className="w-full px-6 md:px-10 pt-6 md:pt-8 flex items-center justify-between">
        <a
          href="/"
          className="flex items-center gap-2 text-slate-200 font-medium uppercase tracking-widest text-sm sm:text-base"
        >
      ...
      <div className="px-5 sm:px-8 md:px-10 pt-12 sm:pt-20 md:pt-24 pb-16">
        {state.phase === 'idle' && (
          <>
            ...
            >
              ...
              <p
                className="max-w-2xl font-light text-slate-400 leading-relaxed"
                style={{ fontSize: 'clamp(1rem, 1.7vw, 1.25rem)' }}
              >
                Drop in your resume and target job title. Get a professional, AI-powered review with
            ...
            <div className="mt-16 sm:mt-20 max-w-3xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 text-center">
              {[
                ...
                <div
                  key={stat.label}
                  className="rounded-2xl border border-slate-700 bg-slate-800/50 p-5 flex flex-col items-center gap-2"
                >
        ...
        {state.phase === 'error' && (
          <div className="max-w-xl mx-auto flex flex-col items-center gap-6 py-16 text-center">
            ...
            <p className="text-slate-400 leading-relaxed">{state.error}</p>
            <button
              type="button"
              onClick={reset}
              className="inline-flex items-center gap-2 rounded-full border-2 border-slate-600 px-8 py-3 text-sm font-medium uppercase tracking-widest text-slate-300 hover:bg-slate-800 transition-colors"
            >
              Try again
            </button>
          </div>
      ...
      {/* Footer */}
      <footer className="border-t border-slate-800 px-6 md:px-10 py-6 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs uppercase tracking-widest text-slate-500">
        <span>© 2026 ProfilePilot AI</span>
