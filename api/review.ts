...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    roleRelevance: {
      type: 'string',
      description: 'A brief evaluation of how well the resume aligns with the target job title (if provided), or a general fit statement if not provided.',
    },
  },
  required: ['overallScore', 'categoryScores', 'summary', 'strengths', 'weaknesses', 'rewrites', 'roleRelevance'],
};
...
If a target job title is provided, tailor your evaluation to see how well the resume aligns with that role. If not provided, evaluate general best practices.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { resumeText, jobTitle } = (req.body ?? {}) as { resumeText?: unknown; jobTitle?: unknown };
